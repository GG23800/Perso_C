...
#define Rmin 120 //minimum distance (mm) of the scan
#define Rmax 220 //maximum distance (mm) of the scan
...
void writefile (double **x, double **y, int **z, int line, int row, char *name)
{
	int i, j;
	FILE * f;
	f = fopen(name,"w+");
...
int main(int arg, char *argv[])
{
	...
	{	
		...
		//sectorial plot
		gnuplot_angle_gray_int(h, x, y, z, BuffLength-1, Nline);

		//save each 10 images
		if (l/10*10==l)
		{
			sprintf(name, "image%i.txt",l);
			writefile(x,y,z,Nline,BuffLength-1,name);
		}
		l++;
	}
