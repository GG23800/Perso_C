...
void send_int16_TCP_server(client* client_list, int16_t *buffer, int buff_length, int target); //same as send_TCP_server but the variable sent are coded on 2 bytes (16 bits int) instead only one use with receive_int16_TCP_client
void reveive_int16_TCP_client(SOCKET* sock, int16_t * buffer, int buff_length); //receive 16 bits buffer of size buff_length from server, use with send int16_TCP_server
...
void send_int16_TCP_server(client* client_list, int16_t *buffer, int buff_length, int target)
{
	int i=0;
	...
	if (target<0)
	{
		for (i=0 ; i<client_list->NbClient ; i++)
		{
			if(send(client_list->sock_client[i], tmp, 2*buff_length, 0)==0)
			{
				clear_client(client_list, client_list->id_client[i]);
				printf("client %u disconnected\n",client_list->id_client[i]+1);
			}
	...
	{
		if(send(client_list->sock_client[target], tmp, 2*buff_length, 0)<0)
		{
			perror("send()");
			exit(errno);
		}
...
void receive_int16_TCP_client(SOCKET* sock, int16_t *buffer, int buff_length)
{
	if (recv((*sock), (char *)buffer, 2*buff_length+1, MSG_WAITALL)==0)
	{
		printf("server closed\n");
	}
}
