...
struct data
{
	...
	stepper_motor stepper={.pin_en=0, .pin_ms1=0, .pin_ms2=0, .pin_ms3=0, .pin_step=0, .pin_dir=0, .step_size=full};
	char *buffer_char;
	float *buffer_float;
	int16_t *buffer_int16;
	int mode; //0 if mesure raw data and send int16_t, 1 for envelope and send char
	int step; //0 no stepper plug to RedPitay, 1 drive stepper
};
...
void send_int16_via_tcp(int line, int16_t *buffer_int16, float *buffer_float, uint32_t buffer_length, client *client_list)
{
	...
	{
		buffer_int16[i+1]=(int16_t)(buffer_float[i]/20.0)*8192;
	}
...
{
	int i=0;
	int16_t tmp_buffer[(int)bufferlength*2];
	for (i=0 ; i<(int)buffer_length ; i++)
	{
