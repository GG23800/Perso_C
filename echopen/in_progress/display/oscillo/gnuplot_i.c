...
In the idea of make an ap for echopen, it is possible to have x=r*cos(theta) and y=r*sin(theta) and one must use gnuplot_angle_grey_IMP, x and y must be a 2D tensor (2D table). enjoy
*/
...
void gnuplot_double_plot(gnuplot_ctrl *handle, int Nb_point, double *x, double *y1, double *y2)
{
	int i=0;
	if (handle==NULL || x==NULL || y1==NULL || y2==NULL || (Nb_point<1)) return ;

	gnuplot_cmd(handle, "plot '-' title 'voie 1' with lines,\\");
	gnuplot_cmd(handle, "'-' title 'voie 2' with lines");

	for (i=0 ; i<Nb_point ; i++)
	{
		gnuplot_cmd(handle, "%.18e %.18e", x[i], y1[i]);
	}
	gnuplot_cmd(handle, "e");
	for (i=0 ; i<Nb_point ; i++)
	{
		gnuplot_cmd(handle, "%.18e %.18e", x[i], y2[i]);
	}
	gnuplot_cmd(handle, "e");
