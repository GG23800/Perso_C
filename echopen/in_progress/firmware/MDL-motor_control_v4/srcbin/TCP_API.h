...
//#include"echopenRP.h" //for changing ramp
...
// prototype for function declare on echopenRP.h
...
void add_client_ramp(client* client_list, SOCKET sock_server); //TCP server is initialised it add new client informations in structure client
...
void *TCP_server_get_ramp(void* p_data); //server routine function for thread, server to get ramp
void launch_server(SOCKET* sock, client* client_list); //function that launch the server in parallel to main
void launch_setting(SOCKET* sock, client* client_list); //function that launch the server to get ramp
void close_TCP_server(SOCKET* sock, client* client_list); //close client connexion and TCP server (for server)
void close_TCP_server_get_ramp(SOCKET* sock, client* client_list); //close client connexion and TCP server (for server)
...
void add_client_ramp(client* client_list, SOCKET sock_server)
{
                unsigned int socklen=sizeof(SOCKADDR_IN);
                SOCKET tmp;
		int Nset=2;
		...
		tmpint=(int)(255.0*level0);
                buffer[0]=(char)tmpint;
		tmpint=(int)(255.0*levelf);
                buffer[1]=(char)tmpint;
                ...
                if (client_list->NbClient<=client_list->Nmax)
                {
                        ...
                        else
                        {
                                send_TCP_server(client_list, buffer, Nset, client_list->NbClient);
                                client_list->NbClient+=1;
                                printf("Client number %i on %i, connected on socket ramp = %i\n",client_list->NbClient,client_list->Nmax,client_list->sock_client[client_list->NbClient-1]);
                        }
                }
                //else{printf("too many clients\n");}
}
...
void *TCP_server_get_ramp(void* p_data)
{
        server_info* serv_info=p_data;
	char buffer[2];
	//double gain0=0.0, gainf=0.0;

        while (1)
        {
                add_client_ramp(serv_info->client_list,serv_info->sock);
		printf("client number %i connected to settings\n",serv_info->client_list->NbClient);
		while (1)
		{
			if (receive_TCP_server(serv_info->client_list, buffer, 2, 0))
			{
				printf("close setting socket\n");
				break;
			}
			else
			{
				level0=((double)int_converter(buffer[0]))/255.0;
				levelf=((double)int_converter(buffer[1]))/255.0;
				printf("gain at 0 = %f, gain at end = %f\n", level0, levelf);

				// change ramp settings
				end_ramp();
				init_ramp();
				ramp();
			}
...
void launch_setting(SOCKET* sock, client* client_list)
{
        server_info* serv_info=(server_info *)malloc(sizeof(server_info));
        serv_info->sock=(*sock);
        serv_info->client_list=client_list;

        if(pthread_create(&TCP_ramp, NULL, TCP_server_get_ramp, serv_info)!=0)
        {
                perror("pthread_create()");
                exit(errno);
        }
...
void close_TCP_server_get_ramp(SOCKET* sock, client* client_list)
{
        int i=0;

        pthread_cancel(TCP_ramp);//close thread
