...
void init_data(data *data_RP, data *data_ramp, unsigned int Nmax, int PORT, float level0, float levelf, mode stepper_mode); //initialise data structure
void clear_data(data *data_RP, data *data_ramp); //clear data structure
...
void init_data(data *data_RP, data *data_ramp, unsigned int Nmax, int PORT, float level0, float levelf, mode step_mode)
{
	...
	//init ramp server
	data_ramp->client_list=(client *)malloc(sizeof(client));
        init_TCP_server(&(data_ramp->sock), PORT+1, data_ramp->client_list, Nmax);
        launch_setting(&data_ramp->sock, data_ramp->client_list);
	...
	if (step==1)
	{
		...
		init_position(data_RP->stepper, 140.0-sector/2.0);
	}
}

void clear_data(data *data_RP, data *data_ramp)
{
	//clear stepper
	if (step==1)
	{
	...
	//close clear ramp server
	close_TCP_server_get_ramp(&data_ramp->sock, data_ramp->client_list);
        free(data_ramp->client_list);
...
void init_ramp()//(int trigger, float level0, float r0, float levelf, float rf) 
{
	...
	if(loc_r0<0.0){loc_r0=0.0;}
	if(loc_rf>rmax){loc_rf=rmax;}
	...
	int N0=(int)(loc_r0/lsamp); //r0 position in the buffer
	int Nf=(int)(loc_rf/lsamp); //rf position in the buffer
